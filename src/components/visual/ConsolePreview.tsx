...
export const ConsolePreview: React.FC = () => {
  return (
    <div className="cs-console-preview" aria-hidden>
      <div className="flex items-center gap-2 h-9 px-3 border-b border-[var(--cs-line)] bg-[var(--cs-muted)]">
        ...
        <span className="ml-2 h-6 flex-1 rounded-[4px] bg-white border border-[var(--cs-line)] text-[11px] text-[var(--cs-ink-3)] px-2.5 flex items-center truncate">
          console.cloudsense.ai/dashboard
        </span>
      </div>

      <div className="grid grid-cols-[72px_1fr] min-h-[320px]">
        <div className="border-r border-[var(--cs-line)] bg-white py-3 px-2 space-y-2">
          {['#e8f0fe', '#f1f3f4', '#f1f3f4', '#f1f3f4', '#f1f3f4'].map((bg, i) => (
            <div key={i} className="h-7 rounded-[4px]" style={{ background: bg }} />
          ))}
        </div>
        ...
        <div className="p-3 bg-[var(--cs-bg)] space-y-3">
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={m.label} className="bg-white border border-[var(--cs-line)] rounded-[4px] p-2">
                <p className="text-[10px] text-[var(--cs-ink-3)]">{m.label}</p>
                <p className="text-[13px] font-medium tabular-nums mt-0.5">{m.value}</p>
                <p className="text-[10px] text-[var(--cs-ink-3)]">{m.hint}</p>
              </div>
            ))}
          </div>

          <div className="bg-white border border-[var(--cs-line)] rounded-[4px] p-2.5">
          ...
          <div className="bg-white border border-[var(--cs-line)] rounded-[4px] px-2.5 py-2 flex items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="text-[11px] font-medium truncate">Resize payment-processing-worker-04</p>
